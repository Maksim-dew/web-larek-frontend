...
#### Класс BasketCard
Расширяет класс Card.Отвечает за отображение краткой информации о товаре в списке товаров корзины.

- `itemIndex: HTMLElement` - порядковый номер товара в корзине
- `basketList: HTMLElement` - элемент-контейнер списка товаров
- `basketPrice: HTMLElement` - элемент с текстом об общей стоимости товаров в корзине
Методы:
- `delBasket(): void` - метод удаления карточки из корзины.
- `registerBasket(): void` - метод оформления заказа.
- `get form: HTMLElement` - геттер для получения элемента формы

#### Класс Order
Расширяет класс Form. Отвечает за отображение первой формы заказа.
...
Методы:
- `next(): void` - метод перехода на следующее окно.
- `get form: HTMLElement` - геттер для получения элемента формы
...
Методы:
- `pay(): void` - метод завершения заказа.
- `get form: HTMLElement` - геттер для получения элемента формы
